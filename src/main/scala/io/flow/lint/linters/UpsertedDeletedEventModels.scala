...
case object UpsertedDeletedEventModels extends Linter with Helpers {
  ...
  override def validate(service: Service): Seq[String] = {
    service.models.filterNot { m => LegacyModels.contains(m.name) }.flatMap { m =>
      parse(m.name) match {
        case None => Nil
  ...
  private[this] val LegacyModels: Set[String] = Set(
    ...
    "blacklisted_item_deleted",
    "item_dimension_estimate_deleted",
    "item_dimension_estimate_upserted",
    "label_invoice_deleted",
    "label_invoice_upserted",
