...
object Expansions extends Helpers {
  ...
  /** Accepts a list of field types, returning the list of possible examples.
    *
    * Examples: Seq("id", "name") => Nil Seq("id", "expandable_organization") => Seq("organization")
    */
  ...
  private[this] def toName(field: String): Option[String] = {
    ...
    val name = if (idx < 0) { baseType(field) }
    else { baseType(field.substring(idx + 1)) }
