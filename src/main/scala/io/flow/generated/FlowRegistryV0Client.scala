 ...
 * apibuilder:0.12.38 https://app.apibuilder.io/flow/registry/0.1.31/ning_1_9_client
 */
package io.flow.registry.v0.models {
   ...
   * @param dependencies A list of the direct dependencies of this application.
   * Guaranteed to be non-cyclical over the set of all applications.
   */
  ...
  /**
   * @param internal The internal port number that this application runs on. Defaults
   * to the service's default port number.
   * @param dependency The application IDs on which this application is dependent
   * @param service
   * @param id
   * @param external The external port number to allocate to this application.
   * Defaults to a unique port assigned by the registry.
   */
  ...
  /**
   * @param service
   * @param external The port on which clients access this service. If running in a
   * container, this is the host port.
   * @param internal The port on which this service is running internally. If running
   * in a container, this is the port inside the container.
   */
  case class Port(
...
package io.flow.registry.v0 {
  ...
  object Constants {
    ...
    val UserAgent = "apibuilder:0.12.38 https://app.apibuilder.io/flow/registry/0.1.31/ning_1_9_client"
    val Version = "0.1.31"
    val VersionMajor = 0
    ...
    object Applications extends Applications {
      ...
      override def post(
        ...
        _executeRequest("POST", s"/applications", body = Some(payload), requestHeaders = requestHeaders).map {
          ...
          case r if r.getStatusCode == 401 => throw new io.flow.registry.v0.errors.UnitResponse(r.getStatusCode)
          case r if r.getStatusCode == 422 => throw new io.flow.registry.v0.errors.GenericErrorResponse(r)
      ...
      override def putDependenciesByIdAndDependency(
        ...
        _executeRequest("PUT", s"/applications/${_root_.io.flow.registry.v0.PathSegment.encode(id, "UTF-8")}/dependencies/${_root_.io.flow.registry.v0.PathSegment.encode(dependency, "UTF-8")}", requestHeaders = requestHeaders).map {
          ...
          case r if r.getStatusCode == 404 => throw new io.flow.registry.v0.errors.UnitResponse(r.getStatusCode)
          case r if r.getStatusCode == 422 => throw new io.flow.registry.v0.errors.GenericErrorResponse(r)
          case r => throw new io.flow.registry.v0.errors.FailedRequest(r.getStatusCode, s"Unsupported response code[${r.getStatusCode}]. Expected: 200, 401, 404, 422", requestUri = Some(r.getUri.toJavaNetURI))
        }
      }
  ...
  trait Applications {
     ...
     * @param service
     * @param prefix Find the applications with this exact prefix (computed as the
     * string up the last dash, of if no dash, the full string)
