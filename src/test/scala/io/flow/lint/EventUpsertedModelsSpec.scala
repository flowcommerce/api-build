...
class EventUpsertedModelsSpec extends AnyFunSpec with Matchers {
  ...
  it("fields") {
    linter.validate(buildService(Seq("event_id"))) should be(
      Seq(
        "Model org_upserted: timestamp field is required in event models"
      )
    )

    linter.validate(buildService(Seq("event_id", "foo", "timestamp"))) should be(
      Seq(
    ...
    linter.validate(buildService(Seq("event_id", "timestamp", "number"))) should be(
      Seq(
    ...
    linter.validate(buildService(Seq("event_id", "timestamp", "foo", "organization"))) should be(
      Seq(
        "Model org_upserted: organization field must come after timestamp in event models"
      )
    )

    linter.validate(buildService(Seq("event_id", "timestamp", "id", "foo", "organization"))) should be(
      Seq(
    ...
    linter.validate(buildService(Seq("event_id", "timestamp", "id", "organization", "foo", "number"))) should be(
      Seq(
    ...
    linter.validate(buildService(Seq("event_id", "timestamp", "organization", "foo", "number"))) should be(
      Seq(
        "Model org_upserted: number field must come after organization in event models"
      )
    )
